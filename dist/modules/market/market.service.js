...
class MarketService {
    async getAll() {
        ...
        if (!etfs || etfs.length === 0)
            throw new AppError("ETFs not found", 404);
        return etfs;
    }
    ...
    async getAllQuotes() {
        ...
        const quotes = await Promise.all(etfs.map(async (etf) => {
            try {
    ...
    async getDetailsByEtfId(etf_id) {
        if (!etf_id)
    ...
    //     }))
    // }
    async getEtfPerformance(symbol, period = '1M') {
        if (!symbol)
            throw new AppError("symbol requis", 400);
        const config = {
...
const formatPerformance = (prices) => {
    if (!prices?.length)
        return [];
