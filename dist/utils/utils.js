...
export const comparePasswords = async (hashed_pswd, plain_pwsd) => {
    const is_same = await bcrypt.compare(plain_pwsd, hashed_pswd);
    return is_same;
};
export function generateTimestamps(start, end, intervalMs) {
    const timestamps = [];
...
export async function getXafToUsdRate() {
